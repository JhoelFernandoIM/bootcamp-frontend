...
function elementosEnComun(arr1 = [], arr2 = []) {
  return [...new Set(arr1.filter(elemento => arr2.includes(elemento)))];
}
...
function caracterFrecuente(texto = '') {
    let caracteres = texto.replace(/\s/g, '').split('');
    let contador = {};
    ...
    for (let caracter in contador) {
        if (contador[caracter] > mayor) {
            mayor = contador[caracter];
            resultado = caracter;
        }
    }
    return resultado;
...
function obtenerLongitudes(texto = '') {
    ...
    return texto.trim().split(/\s+/).map(palabra => palabra.length)
}
...
function promedioArray(arr = []) {
    ...
    let suma = arr.reduce((total, numero) => total + numero, 0);
...
function textoALista(texto = '') {
    ...
    return texto.split(',').map(palabra => palabra.trim());
}
